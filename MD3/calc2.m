function [thrust, fuelFlow] = calc2(BR,pfr,T04, areaIn, alt)

% Atmospheric Conditions
[Tatm, ~, Patm, rhoAtm] = atmosisa(alt);
...
uef=(2.0.*nfn.*(gammafn./(gammafn-1.0)).*R.*T08.*(1-(Patm./P08).^((gammafn-1.0)./gammafn))).^0.5;
...
ce=(gamman.*R.*T7) .^0.5;
ue=(2.0.*nn.*(gamman./(gamman-1.0)).*R.*T06.*(1-(Patm./P06).^((gamman-1.0)./gamman))).^0.5;
Me=ue./ce;
...
%outputs
fuelFlow = f .* (rhoAtm .* V .* areaIn);
thrust = areaIn .* st .* rhoAtm .* V;
end